# 📚 Coaching Institute Website (Frontend)

This is a modern, responsive **Coaching Institute Website frontend** designed for **Class 11–12, JEE, NEET, and Commerce students**.  
The project focuses on a clean UI, student-friendly layout, and demo login functionality.
Its a demo and a project website to just showcase and give a rough idea for main frontend.

🔗 **Live Website:**  


---

## ✨ Features

- 🏫 Professional coaching institute landing page
- 📱 Fully responsive (mobile, tablet, desktop)
- 🎓 Student login system (demo-based)
- 🧠 Login state managed using `localStorage`
- 🚪 Logout functionality
- 📄 Sections for courses, batches, faculty & contact
- ⚡ Fast static hosting via GitHub Pages

---

## 🛠️ Tech Stack

- **HTML5** – Page structure  
- **CSS3** – Styling & responsiveness  
- **JavaScript (Vanilla JS)** – Logic & DOM handling  
- **GitHub Pages** – Free hosting  

---

## 🔐 Demo Login Credentials

> This project uses **frontend-only authentication** for UI/demo purposes.

**Student Login**
